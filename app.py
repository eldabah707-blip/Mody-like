...
async def generate_jwt_token(uid, password):
    ...
    try:
        ...
        url = f"https://mody-panel-jwt.vercel.app/token?uid={uid}&password={encoded_password}"
...
def handle_requests():
    ...
    # After likes: the game/profile service may update the Likes counter a
    # little later than the LikeProfile request.  Verify more than once before
    # declaring that zero likes were given.
    after = None
    ...
    for attempt in range(verify_attempts):
        ...
        if after is not None:
            try:
                ...
                candidate_like = int(after_data['AccountInfo'].get('Likes', before_like))
                after_like = candidate_like
                if candidate_like > before_like:
                    break
            except Exception:
                pass
        if attempt < verify_attempts - 1:
            # Give the backend time to propagate the new counter.
            time.sleep(1.5)
    ...
    try:
        after_data = json.loads(MessageToJson(after))
        after_like = int(after_data['AccountInfo'].get('Likes', after_like))
